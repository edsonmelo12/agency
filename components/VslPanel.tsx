...
const VslPanel: React.FC<Props> = ({ script, uiTheme, isLoadingAudio, setIsLoadingAudio }) => {
  ...
  if (!script) {
    return (
      <div className="flex-1 flex items-center justify-center bg-paper p-8">
        <div className="max-w-md text-center space-y-6 opacity-70">
          <div className="w-20 h-20 bg-panel rounded-full flex items-center justify-center mx-auto mb-4 shadow-card border border-border">
          ...
          <h3 className="text-xl font-display uppercase tracking-tight">VSL Studio: Roteirização</h3>
          <p className="text-xs font-bold leading-relaxed text-slate">Gere um script de vendas magnético para transformar visitantes em compradores através de vídeo.</p>
        </div>
      </div>
    );
  }
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (isTeleprompter) setIsTeleprompter(false);
    };
  ...
  return (
    <div className={`flex-1 flex flex-col h-full overflow-hidden transition-all duration-500 ${isTeleprompter ? 'bg-black' : 'bg-paper'}`}>
      {/* Header do Painel */}
      <div className={`h-16 border-b px-8 flex items-center justify-between z-50 ${isTeleprompter ? 'border-white/10 bg-black' : 'border-border bg-panel shadow-sm'}`}>
        <div className="flex items-center gap-3">
           <span className={`text-[10px] font-black uppercase tracking-widest px-2 py-1 rounded ${isTeleprompter ? 'bg-white/10 text-white' : 'bg-primary text-white'}`}>
             {script.model} Mode
           </span>
           <h3 className={`text-sm font-display uppercase ${isTeleprompter ? 'text-white' : 'text-ink'}`}>{script.title}</h3>
        </div>
        <div className="flex items-center gap-4">
           {!isTeleprompter && (
             <button 
              ...
              className="text-[10px] font-black uppercase text-primary hover:underline flex items-center gap-2 disabled:opacity-50"
             >
           ...
           <button 
            ...
            className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase transition-all ${isTeleprompter ? 'bg-rose-600 text-white' : 'bg-ink text-white'}`}
           >
               ...
               if (line.startsWith('[') && line.endsWith(']')) {
                 return !isTeleprompter ? (
                   <div key={idx} className="p-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-100 dark:border-blue-800 rounded-xl">
                      <span className="text-[10px] font-black uppercase text-primary block mb-1">Cena / Visual</span>
                      <p className="text-xs font-bold text-blue-900 dark:text-blue-100 italic">{line}</p>
                   </div>
                 ) : null;
               }
               return line.trim() && (
                 <p key={idx} className={`leading-relaxed transition-all duration-300 ${isTeleprompter ? 'text-4xl md:text-6xl font-black text-white' : 'text-lg font-medium text-slate'}`}>
                    {line}
                 </p>
               );
             })}
